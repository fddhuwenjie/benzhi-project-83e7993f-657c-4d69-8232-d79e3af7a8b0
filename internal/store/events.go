...
func (s *SQLiteStore) ChainHead(ctx context.Context, caseID string) (string, error) {
	var digest string
	err := s.db.QueryRowContext(ctx, `SELECT digest FROM audit_events WHERE case_id=? ORDER BY sequence DESC LIMIT 1`, caseID).Scan(&digest)
	if errors.Is(err, sql.ErrNoRows) {
		return "", ErrNotFound
	}
	return digest, err
...
func (s *SQLiteStore) Events(ctx context.Context, caseID string) ([]domain.AuditEvent, error) {
	rows, err := s.db.QueryContext(context.Background(), `SELECT sequence,kind,actor_id,occurred_at,payload,previous_digest,digest FROM audit_events WHERE case_id=? ORDER BY sequence`, caseID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
