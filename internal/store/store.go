...
type SQLiteStore struct {
	...
	verificationResults map[string]dossierVerification
}
...
func Open(path string) (*SQLiteStore, error) {
	...
	return &SQLiteStore{db: db, verificationResults: map[string]dossierVerification{}}, nil
}
...
func (s *SQLiteStore) cachedVerification(caseID string) (dossierVerification, bool) {
	s.verificationMu.RLock()
	defer s.verificationMu.RUnlock()
	result, ok := s.verificationResults[caseID]
	return result, ok
}

func (s *SQLiteStore) rememberVerification(caseID string, result dossierVerification) {
	s.verificationMu.Lock()
	defer s.verificationMu.Unlock()
	s.verificationResults[caseID] = result
}
