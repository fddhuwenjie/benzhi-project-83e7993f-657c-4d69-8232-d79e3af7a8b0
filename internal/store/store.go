...
func Open(path string) (*SQLiteStore, error) {
	...
	return &SQLiteStore{db: db, chainHeads: make(map[string]string)}, nil
}
...
func (s *SQLiteStore) Create(ctx context.Context, c *domain.ReconstructionCase, event EventInput) error {
	...
	head, err := s.appendEvent(ctx, tx, c.CaseID, event)
	if err != nil {
		return err
	}
	...
	s.rememberChainHead(c.CaseID, head)
	return nil
}
...
func (s *SQLiteStore) cachedChainHead(caseID string) string {
	s.chainHeadsMu.Lock()
	defer s.chainHeadsMu.Unlock()
	return s.chainHeads[caseID]
}

func (s *SQLiteStore) rememberChainHead(caseID, head string) {
	s.chainHeadsMu.Lock()
	defer s.chainHeadsMu.Unlock()
	s.chainHeads[caseID] = head
}
