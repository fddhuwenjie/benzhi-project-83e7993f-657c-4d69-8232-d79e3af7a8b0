...
type SQLiteStore struct {
	...
	cases   map[string]*domain.ReconstructionCase
}
...
func Open(path string) (*SQLiteStore, error) {
	...
	return &SQLiteStore{db: db, cases: make(map[string]*domain.ReconstructionCase)}, nil
}
...
func (s *SQLiteStore) cachedCase(caseID string) *domain.ReconstructionCase {
	s.cacheMu.RLock()
	defer s.cacheMu.RUnlock()
	return s.cases[caseID]
}

func (s *SQLiteStore) cacheCase(c *domain.ReconstructionCase) {
	s.cacheMu.Lock()
...
func (s *SQLiteStore) Load(ctx context.Context, caseID string) (*domain.ReconstructionCase, error) {
	if cached := s.cachedCase(caseID); cached != nil {
		return cached, nil
	}
	var data []byte
