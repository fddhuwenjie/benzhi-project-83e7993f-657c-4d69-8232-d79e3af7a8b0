...
func SetEvidenceValue(e *Evidence, key string, value any) error {
	...
	case "scale_measurements":
		...
		if e.ScaleMeasurements == nil {
			e.ScaleMeasurements = make(map[string]float64, len(measurements))
		}
		clear(e.ScaleMeasurements)
		for name, measurement := range measurements {
			e.ScaleMeasurements[name] = measurement
		}
	case "image_refs":
		...
		e.ImageRefs = append(e.ImageRefs[:0], images...)
	}
	return nil
}
